...
class Param:
    def __init__(self, x=None, min=None, max=None, mean=None,
                 stdev=None):
        if min is not None and max is not None:
            self.value = lambda: np.random.uniform(min, max)
        elif mean is not None and stdev is not None:
            self.value = lambda: np.random.normal(mean, stdev)
        elif type(x) is list:
            self.values = x
            self.value = lambda: np.random.choice(self.values)
        elif type(x) is function:
            self.value = x
        elif type(x) in (str, int, float):
            self.value = lambda: x
        # else:
        #     TODO error
